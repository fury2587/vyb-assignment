...
<section className="container mx-auto p-4 mt-8">
  ...
  <nav className="flex justify-between items-center bg-gray-800 p-4 rounded-lg">
    ...
    <Link href="/merch" className="text-white hover:text-green-400 px-4">Merch</Link>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
// Corrected duplicate 'Delhi' entry
            { name: 'Darjeeling', state: 'West Bengal', image: '/darjeeling.png' },
            { name: 'Munnar', state: 'Kerala', image: '/munnar.png' },
            { name: 'Delhi', state: 'Delhi', image: '/delhi.png' },
            { name: 'Agra', state: 'Uttar Pradesh', image: '/agra.png' } // Assuming you want a different place here
          ].map((place, i) => (
            <div key={i} className="relative">
              <Image src={place.image} alt={place.name} width={400} height={400} className="w-full h-auto object-cover rounded-lg" />
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black to-transparent p-4">
        ...
        <div className="relative">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="flex items-center space-x-8">
              {[
                { step: '01', text: 'Create Itinerary' },
                { step: '02', text: 'Add to Store' },
                { step: '03', text: 'Promote Your Store' },
              ].map((item) => (
                <div key={item.step} className="text-center">
                  <p className="text-4xl font-bold text-green-400">{item.step}</p>
      ...
      <section className="container mx-auto p-4 mt-8">
        ...
        <div className="bg-gray-800 rounded-lg p-4">
          {[
            { question: 'What is VYB Marketplace?', answer: 'VYB Marketplace is an online platform that allows influencers to create and sell itineraries, merchandise, and digital products.' },
            { question: 'How do I create an itinerary?', answer: 'You can create itineraries by signing up and following the step-by-step guide on our platform.' },
            { question: 'Can I promote my store?', answer: 'Yes, we provide various tools to help you promote your store and reach your audience.' },
            { question: 'Is there a fee to join?', answer: 'Joining VYB Marketplace is free, but there may be fees for certain premium features.' },
          ].map((item, index) => (
            <div key={index} className="mb-2">
              <button className="w-full text-left bg-gray-700 px-4 py-2 rounded-lg" onClick={() => toggleFaq(index)}>
                <h3 className="flex justify-between items-center">
                  <span className="font-semibold">{item.question}</span>
                  <span>{expandedFaq === index ? '-' : '+'}</span>
                </h3>
              </button>
              {expandedFaq === index && <p className="mt-2 px-4">{item.answer}</p>}
            </div>
          ))}
        </div>
      </section>
